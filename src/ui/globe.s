...
;;
;; Кадр (render): зумы 0–2 — готовый вид с карты (globe_view.s) -> тень -> вывод отрезков
;; (globe_s.s _gl_rows_pre); та же геометрия и новое солнце — только тень и проход строк;
;; иначе рёберный путь: таблицы произведений на 8 констант вида -> центры ячеек 30° -> отсев
...
2$:	ld	(ocean), a
	...
	ld	(bs_to), hl
	call	build_strips
3$:	; ---- предрасчитанный вид (зумы 0–2): геометрии нет — тень и вывод готовых отрезков
	ld	a, (_gv_off)
	or	a, a
	jp	nz, r_sunonly
	ld	hl, (r_lon)
...
7$:	ld	(_gl_gtex), a
	...
	call	bands
	call	r_shadow
	ld	a, (r_z)			; запись отрезков — сразу только на зумах 0–2 (рёберный путь там —
	cp	a, #3				; вид вне сетки, AEL дорогой); на 3–5 — с первой смены солнца,
	ld	a, #0				; поворотам она не нужна
	adc	a, a
	ld	(_gl_capwant), a
	call	_gl_rows
	ld	a, (work)
...
6$:	ld	hl, #s_nopg
	...
	pop	ix
	ret
7$:	ld	a, (epage)			; предрасчитанные виды с карты (зумы 0–2)
	push	af
	inc	sp
	ld	e, #b_gview_open
	ld	hl, #_gview_open
