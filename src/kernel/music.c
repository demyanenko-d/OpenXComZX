...
// Самая длинная запись потока: байт n0, 127 пар, байт n1, 255 пар (22 §4.3). Запас должен
// быть не меньше её, иначе запись может начаться в зоне чтения и не поместиться целиком.
#define REC_MAX     768
...
void mus_fill(void) __banked
{
	...
	for (;;) {
		...
		// Запись не помещается в остаток разрешённой зоны — перечитать половину с этой же
		// позиции. Раньше здесь ещё и двигали M.pos на остаток: кусок потока выбрасывался,
		// дальше байты данных разбирались как команды — в чип летел мусор (хрип), а из
		// случайных пауз плыл темп. Проявлялось раз в 8 КБ потока, то есть раз в несколько
		// секунд музыки.
