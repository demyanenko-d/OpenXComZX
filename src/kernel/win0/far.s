...
1$:	res	0, e
	...
	;; памяти и затирает ядро под собой — машина умирает целиком. Длину чиним, а в консоль
	;; пишем, кто звал, приёмник, источник, длину и верх стека — но только первые три раза:
	;; глобус зовёт far_copy каждый кадр, и бесконечная печать сама роняла машину по стеку
	;; (findings_log, 2026-09-26).
	ld	a, h
	or	a, l
	jp	z, fc_ok
	ld	a, (fc_rep)
...
fc_spl:	push	bc
	...
	call	_dbg_puts
	pop	iy
	;; Чиним длину и работаем дальше: в игре копий длиннее #FFFF нет, а младшее слово
	;; приходило верным во всех замеченных случаях — так сбой вообще не виден снаружи.
fc_fix:
	ld	hl, #0
	ld	(fc_len + 2), hl
	jp	fc_ok

;; Сколько отчётов о мусорной длине ещё печатать (лежит в коде — это тоже ОЗУ)
fc_rep:	.db	3
