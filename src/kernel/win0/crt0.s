...
KERNEL_PAGE	= 0x04
IM2_I		= 0x3E
STACK_TOP	= 0x3EF0		; выше — векторы IM2 (#3EFB-#3F00), их трогать нельзя
;; Ниже этой границы под стеком остаётся кольцо музыки (#1E00-#2E0F): если стек дойдёт
;; туда, подкачка с карты и прерывание музыки затрут адреса возврата, и машина уедет в
;; никуда — видели это не раз (findings_log, 2026-09-26). Сторож в прерывании ловит
;; приближение и честно падает с сообщением, вместо тихой порчи памяти.
STACK_GUARD	= 0x31			; старший байт SP: ниже — авария. #3000-#300A — переменные
;; плеера (mus_rd, mus_out и прочее): затрёшь их — прерывание уйдёт по случайному адресу
...
_start::
	...
	;; Область стека заполняется меткой #A5: видно, докуда он проседал (peek в сценарии),
	;; и автотест проверяет, что до кольца музыки под ним стек не добрался (22 §10.9).
	;; В геоскейпе стек уходит примерно на 2 КБ — кольцо музыки раньше стояло у него на пути.
	ld	hl, #0x3010
	ld	bc, #STACK_TOP - 0x3010
stk_fill:
	ld	(hl), #0xA5
	inc	hl
	dec	bc
	...
	;; обнулить _MUSIC: там кольцо и счётчики плеера, и до первого mus_play их читает
	;; прерывание — иначе оно играет мусор и врёт диагностика (недоборы «с рождения»)
	ld	hl, #s__MUSIC
	ld	bc, #l__MUSIC
	call	zero_mem
...
;; Стек дошёл до кольца музыки: дальше подкачка и прерывание затрут адреса возврата
stack_panic:
	ld	hl, #stack_msg
	ld	bc, #0xF8AF
	jr	panic_loop
...
isr_frame:
	...
	push	de
	push	hl
	;; сторож стека: до кольца музыки осталось меньше страницы — дальше только порча
	ld	hl, #0
	add	hl, sp
	ld	a, h
	cp	a, #STACK_GUARD
