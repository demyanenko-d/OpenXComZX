...
mi_go:
	ld	hl, (_mus_rd)
	ld	d, #2			; заворотов за один вызов — не больше одного
mi_next:
	ld	a, (hl)
	cp	a, #0x80
	...
	dec	d
	jr	nz, mi_next
	;; Два #80 подряд (в начале кольца лежит метка заворота) — кольцо испорчено. Раньше
	;; прерывание крутилось здесь вечно: машина вставала целиком, даже курсор мыши замирал,
	;; потому что прерывания при этом запрещены. Теперь кадр просто пропускаем.
	ld	(_mus_rd), hl
	jr	mi_tick
