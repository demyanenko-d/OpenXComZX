...
#define EVT_MUSIC  10   // верхний экран сменился — поставить свою музыку (аналог State::init
                        //   в оригинале: именно там играет тема экрана, 21 §3)
...
void ui_toggles(void) __banked;         // перерисовать переключатели (скорость времени меняют и события)
...
// Каретка поля ввода — в конец строки (экран сменил содержимое поля сам)
void ui_edit_end(void) __banked;
// Данные под открытыми окнами изменились: сохранённый фон устарел (перерисовать стек при закрытии)
